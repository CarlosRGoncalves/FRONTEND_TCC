...
export default function MiniDrawer (){
  ...
  useEffect(() => {
    async function producao(){
      ...
      const response = axios.get('http://localhost:3006/producao/',{ headers })
      .then(response =>{
  ...
  async function Delete(id){
    ...
    if(window.confirm("Você tem certeza que vai excluir essa Produção?")){
      var result = await axios.delete('http://localhost:3006/producao/'+id,{ headers }).then(res =>{
        if(res.status ===202){
          console.log(res)
  ...
  function cad() {
    window.location.replace("http://localhost:3000/producao/cadastro");
  }
